...
//            $rate = $level_0 + $level_1 + $level_2;
            if ($level_id > 0) {
                ...
                $is_res = $nfx_config->getCommissionIsMax($rate);
            }
            if ($is_res['code'] == -1) {
                return $is_res;
            }
...
//             $level_2 = $_POST["level_2"];
    ...
    public function threeLevelDistributionConfig()
    {
        ...
        $this->assign('child_menu_list', $child_menu_list);
        $commission_service =  new NfxCommissionConfig();
        $commission_rate = $commission_service->getCommissionMaxRate();
    ...
    public function modifyShopConfigIsDistributionOrPromoterIsAudit()
    {
        ...
        $distribution_commission_rate = request()->post("distribution_commission_rate", 0); //推广佣金
        $partner_commission_rate = request()->post("partner_commission_rate", 0); //股东分红佣金
        $regionagent_commission_rate = request()->post("regionagent_commission_rate", 0); //区代佣金

        $shop_config = new NfxShopConfig();
        $retval = $shop_config->modifyShopConfigIsDistributionOrPromoterIsAudit($this->instance_id, $is_open, $is_audit, $is_start, $is_set, $distribution_use, $distribution_commission_rate, $partner_commission_rate, $regionagent_commission_rate);
        return AjaxReturn($retval);
    }
    ...
    public function UpdateShopRegionAgentConfig()
    {
        if (request()->isAjax()) {
            ...
            $nfx_config = new NfxCommissionConfig();
            $rate = $province_rate + $city_rate + $district_rate;
            $is_res = $nfx_config->getCommissionIsMax($rate, 'edit', 'agent', 1);
            if ($is_res['code'] == -1) {
                return $is_res;
            }
            $region_agent = new NfxRegionAgent();
    ...
    public function updatePartnerLevel()
    {
        if (request()->isAjax()) {
            ...
            $nfx_config = new NfxCommissionConfig();
            if ($level_id > 0) {
                $is_res = $nfx_config->getCommissionIsMax($commission_rate, 'edit', 'partner', $level_id);
            } else {
                $is_res = $nfx_config->getCommissionIsMax($commission_rate);
            }
            if ($is_res['code'] == -1) {
                return $is_res;
            }
...
//         $end_time = ! empty($_POST['end_time']) ? $_POST['end_time'] : '';
    ...
    public function updatePartnerLevelAll()
    {
        ...
        foreach ($level_array as $k => $v) {
            $level_array[$k] = explode(",", $v);
            $rate += $level_array[$k][2];
        }
        $nfx_config = new NfxCommissionConfig();

        $is_res = $nfx_config->getCommissionIsMax($rate, 'edit', 'partner', 0);
        if ($is_res['code'] == -1) {
            return $is_res;
        }
        $partner = new NfxPartner();
