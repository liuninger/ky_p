...
class NfxCommissionCalculate extends BaseService implements INfxCommissionCalculate
{
    ...
    private $is_partner_enable; // 股东分红是否开启 partner_id
    private $is_global_enable; // 全球分红是否开启
    private $is_distribution_start; // 分销是否申请

    private $distribution_commission_rate; // 推广佣金比率
    private $partner_commission_rate; // 股东分红比率
    private $regionagent_commission_rate; // 区代比率
...
//                $commission_rate = $goods_commission_rate_model->getInfo(['goods_id' => $order_goods['goods_id']], '*');
                $commission_rate = $shop_config_info;
                if($commission_rate['is_distribution_enable'] == 1){
                    $commission_rate['is_open'] = 1;
                }else{
                    $commission_rate['is_open'] = 0;
                }
        ...
        if (!empty($this->order_info)) {
            ...
            if (!empty($promoter_info)) {
                ...
                if(empty($this->partner_id)){
                    $this->partner_id = $this->getPromoterParent($promoter_info['promoter_id']);
                }
            } else {
                $this->promoter_id = 0;
                $this->partner_id = 0;
...
//        $this->distribution_commission_rate = $shop_config_info["distribution_commission_rate"];
//        $this->partner_commission_rate = $shop_config_info["partner_commission_rate"];
//        $this->regionagent_commission_rate = $shop_config_info["regionagent_commission_rate"];
        $this->distribution_commission_rate = 100;
        $this->partner_commission_rate = 100;
        $this->regionagent_commission_rate = 100;
    }
                    ...
                    if (!empty($commission_rate)) {
                        if ($commission_rate['is_open'])    //启动分销
                        {
                            ...
                            if ($promoter_id != 0) {
                                ...
                                //当前推广员获取佣金
                                $retval = $this->addOrderDistributionCommission($this->shop_id, $promoter_id, $this->order_info['order_id'], $order_goods['order_goods_id'], $order_goods['real_pay'], $order_goods['cost_price'], $order_goods['goods_return'], 0, $this->distribution_commission_rate, $promoter_level_info['level_0']);
                                $this->commission_rate += $promoter_level_info['level_0'];
                                $parent_promoter = $promoter_info['parent_promoter'];
                                if ($parent_promoter != 0) {
                                    $this->addOrderDistributionCommission($this->shop_id, $parent_promoter, $this->order_info['order_id'], $order_goods['order_goods_id'], $order_goods['real_pay'], $order_goods['cost_price'], $order_goods['goods_return'], 1, $this->distribution_commission_rate, $promoter_level_info['level_1']);
                                    $parent_promoter_info = $promoter_model->getInfo(['promoter_id' => $parent_promoter], 'parent_promoter');
                                    $this->commission_rate += $promoter_level_info['level_1'];
                                    $grand_promoter = $parent_promoter_info['parent_promoter'];
                                    if ($grand_promoter != 0) {
                                        $this->addOrderDistributionCommission($this->shop_id, $grand_promoter, $this->order_info['order_id'], $order_goods['order_goods_id'], $order_goods['real_pay'], $order_goods['cost_price'], $order_goods['goods_return'], 2, $this->distribution_commission_rate, $promoter_level_info['level_2']);
    ...
    {
        Log::write('orderPartnerCommission is_partner_enable' . $this->is_partner_enable);
        if ($this->is_partner_enable == 1) {

            $commission_partner = new NfxCommissionPartnerModel();
            Log::write('orderPartnerCommission order_info' . json_encode($this->order_info));
            if (empty($this->order_info)) {
                return 1;
            }
                    ...
                    if (!empty($commission_rate)) {
                        if ($commission_rate['is_open'])    //启动分销
                        {
                            ...
                            if ($partner_id != 0) {
                                ...
                                //下级股东
                                $superior_array = $partner->getPartnerSuperior($partner_id);
                                $partner_commission_rate = 0;

                                $partner_array = array_merge($parents_array, $superior_array);

                                //把相同级别的股东放在一起
                                $new_partner = [];
                                foreach ($partner_array as $k2 => $v2) {
                                    $new_partner[$v2['partner_level']]['data'][] = $v2;
                                    $new_partner[$v2['partner_level']]['level'] = $v2['partner_level'];
                                }
                                $new_partner = array_values($new_partner);

                                $partner_level_model = new NfxPartnerLevelModel();
                                foreach ($new_partner as $k2 => $v2) {
                                    $partner_commission_rate = 0;
                                    $partner_level_model = new NfxPartnerLevelModel();
                                    $partner_level_info = $partner_level_model->get($v2['level']);
                                    $data = $v2['data'];
                                    foreach ($data as $k3=>$v3){

                                        if ($partner_level_info['commission_rate'] > $partner_commission_rate) {
                                            if ($partner_level_info['is_powers'] == 1) {

                                                $powers_rate = sprintf('%.2f', ($partner_level_info['commission_rate'] / count($v2['data'])));

                                                $real_commission_rate = $powers_rate;

                                                $partner_commission_rate += $real_commission_rate;

                                                $this->addOrderPartnerCommission($this->shop_id, $v3['partner_id'], $v3['partner_level'], $partner_id, $this->order_info['order_id'], $order_goods['order_goods_id'], $order_goods['real_pay'], $order_goods['cost_price'], $order_goods['goods_return'], $this->partner_commission_rate, $real_commission_rate);
            ...
            foreach ($new_partner as $k2 => $v2) {
                ...
                foreach ($data as $k3=>$v3){
                    ...
                    if ($partner_level_info['commission_rate'] > $partner_commission_rate) {
                        ...
                        $this->addOrderPartnerCommission($this->shop_id, $v3['partner_id'], $v3['partner_level'], $partner_id, $this->order_info['order_id'], $order_goods['order_goods_id'], $order_goods['real_pay'], $order_goods['cost_price'], $order_goods['goods_return'], $commission_rate['partner_commission_rate'], $real_commission_rate);
                        }else{
                            $real_commission_rate = $partner_level_info['commission_rate'] - $partner_commission_rate;
                            $partner_commission_rate = $partner_level_info['commission_rate'];
    ...
    * @see \data\api\INfxCommissionCalculate::orderPartnerCommission()
    */
    public function oldorderPartnerCommission()
    {
        Log::write('orderPartnerCommission is_partner_enable' . $this->is_partner_enable);
        if ($this->is_partner_enable == 1) {
...
//                            $region_rate = $commission_rate["regionagent_commission_rate"];
                            $region_rate = $this->regionagent_commission_rate;
                            #计算区域代理的金额
    ...
    private function getPromoterParent($promoter_id)
    {
        $promoter_model = new NfxPromoterModel();
        $promoter_info = $promoter_model->getInfo([
            'promoter_id' => $promoter_id
        ], 'promoter_id,parent_promoter');
        if($promoter_info['parent_promoter'] == 0){
            return 0;
        }
        $promoter_info['partner_id'] = 0;
        $partner_model = new NfxPartnerModel();
        $parents_array = array();
        $parents_array[] = $promoter_info;
        $parents_id = 0;
        while ($promoter_info['parent_promoter'] != 0 && $promoter_info['partner_id'] == 0) {
            $promoter_info = $promoter_model->getInfo([
                'promoter_id' => $promoter_info['parent_promoter']
            ], 'promoter_id,parent_promoter');
            $partner_info = $partner_model->getInfo([
                'promoter_id' => $promoter_info['promoter_id']
            ],'partner_id');
            if(empty($partner_info)){
                $promoter_info['partner_id'] = 0;
            }else{
                $promoter_info['partner_id'] = $partner_info['partner_id'];
                $parents_id = $partner_info['partner_id'];
            }
            $parents_array[] = $promoter_info;
        }
        return $parents_id;
