...
class Commission extends BaseController
{
     ...
     * 店铺数据分析
     */
    public function shopData(Time $time, DataAnalyse $dataAnalyse, Request $request)
    {
        if(request()->isAjax()){
            $start_time = $request->post('start_time','');
            $end_time = $request->post('end_time','');
            $start_time = strtotime($start_time);
            $end_time = strtotime($end_time) + 60 * 60 * 24 -1 ;
            $condition = [
                'create_time' => [
                    'between',"$start_time,$end_time"
                ]
            ];
            $order_data = $dataAnalyse->getOrderMoneySum($condition); //订单总金额
            $nca_data = $dataAnalyse->getNcaMoneySum($condition); // 区代佣金
            $ncp_data = $dataAnalyse->getNcpMoneySum($condition); // 股东分红
            $ncd_data = $dataAnalyse->getNcdMoneySum($condition); // 推广佣金
            $appropriate = $nca_data['sum'] + $ncp_data['sum'] + $ncd_data['sum'];
            if($order_data['sum'] > 0){
                $appropriate_rate = sprintf('%.2f',($appropriate / $order_data['sum']) * 100); //拨出百分比
            }else{
                $appropriate_rate = 0.00;
            }


            $actual_money = $order_data['sum'] - $appropriate;
        ...
        $default_time = $time::month();

        $default_time[0] = date('Y-m-d',$default_time[0]);
        $default_time[1] = date('Y-m-d',\time() + 60*24 -1);


        $this->assign('default_time',$default_time);
        return view($this->style . "Commission/shopData");
