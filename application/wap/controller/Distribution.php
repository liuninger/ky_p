...
//         ]);
    ...
    public function applyPartner()
    {
        ...
        $shop = new ShopService();
        $promote_serivce = new NfxPromoter();
        $user_promoter = $promote_serivce->getPromoterInfo($this->uid); //用户推广信息
        $promote_list = $promote_serivce->getPromoterTeamList($user_promoter['promoter_id']); //查询团队列表
        $uid_str = '';
        foreach ($promote_list  as $k=>$v){
            $uid_str .= $v['uid'].',';
        }
        $uid_str = rtrim($uid_str,',');
//        $shop_user_account = $shop->getShopUserConsume($this->shop_id, $this->uid); //用户消费金额
        $shop_user_promoter_account = $shop->getShopUserPromoterConsume($this->shop_id, $uid_str); //团队推广用户消费金额
        $partner_level_list = $nfx_partner->getPartnerLevelAll($this->instance_id);

        $shop_sale_money = 0;
        $is_meet = 0; // 是否满足申请股东最低推广消费金额
